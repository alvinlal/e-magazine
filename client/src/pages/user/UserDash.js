...
export default function UserDash() {
  ...
  const handleLike = async () => {
    ...
    let res = await axios.post(process.env.REACT_APP_SERVER_URL + '/like/likePost', { content_Id: postId }, { withCredentials: true });
    console.log('Res', res);
  ...
  const renderCard = (card, index) => {
    ...
    return (
      <>
        ...
        >
          <CardItem src={card.content_Image} id={card.id} text={card.content_Title} label={card.cat_Name} path='/UserDash' />
          {/* <ThumbDownIcon/>
      <ThumbUpIcon/> */}
        </div>
        ...
        <div>
          <Dialog fullScreen open={open} onClose={handleClose} TransitionComponent={Transition}>
            ...
            <p style={{ margin: '53px 113px', display: 'block', fontSize: 15, lineHeight: '2' }}>{desc}</p>
  ...
  <Box  sx={{maxWidth: 150,display: 'inline', gap: 15 ,width: 120}}>
    <FormControl fullWidth>
      ...
      <div className='grid' style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        {content.map(renderCard)}
